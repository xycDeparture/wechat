...
@Key(['content'])
@Form.create()
export default class TextMessageComp extends React.Component {
    ...
    handlePageChange(nextPage, pageSize) {
        ...
        this.context.router.push({
            pathname: '/wechat/textMessage/list',
            query: query
        })
        this.props.actions.fetchUserMessageList(query)
    }
    ...
    handleSearch(e) {
        ...
        this.props.form.validateFields((errors, values) => {
            ...
            this.props.actions.fetchUserMessageList({page, start_time, end_time, type})
        })
    }

    toReply(id) {
        this.context.router.push({
            pathname: '/wechat/textMessage/reply',
            query: {
                id
            }
        })
    ...
    sendMessage(id) {
        ...
        this.props.actions.sendMessage(data).then(result => {
    ...
    renderTable() {
        ...
        const columns = [{
            ...
            render(_, obj) {
                return(
                    <div>
                        <Auth type={["wechat-text-message-list-check"]}>
                            <a onClick={toggleModal(obj, 'visible_1')} style={{paddingRight: 10}}>查看</a>
                        </Auth>
                        <Auth type={["wechat-text-message-list-reply"]}>
                            <a onClick={toReply(obj.id)} style={{paddingRight: 10}}>回复</a>
                        </Auth>
                        <Auth type={["wechat-text-message-list-send"]}>
                            <a onClick={sendMessage(obj.id)} style={{paddingRight: 10}}>发送</a>
                        </Auth>
                    </div>
                )
            }
